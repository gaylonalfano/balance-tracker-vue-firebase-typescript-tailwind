          ...
          <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
            ...
            <div class="mt-2">
              <p class="text-sm text-gray-500">
                Are you sure you want to delete this member? All data will be
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const { deleteDoc, error, isPending } = useDocument(
      "members",
      props.member.id
    );

    // Handler for deleting member document on click
    async function handleDeleteMember() {
      ...
      if (!error.value) {
        console.log("DELETED member. EMITTING to parent");
        context.emit("close");
      }
    }

    return { error, isPending, handleDeleteMember };
  },
});
</script>
